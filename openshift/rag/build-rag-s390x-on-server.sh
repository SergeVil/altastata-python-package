...
#   SSH_KEY=~/.ssh/my.key SSH_HOST=user@10.0.0.1 ./openshift/rag/build-rag-s390x-on-server.sh
#
...
set -e
# Repo root = directory that contains openshift/ and rag-example/ (altastata-python-package)
...
ssh $SSH_OPTS "$SSH_HOST" "mkdir -p $REMOTE_ALTASTATA_ACCOUNTS/$HPCS_ACCOUNT && cp $REMOTE_DIR/openshift/rag/$HPCS_PROPERTIES_FILE $REMOTE_ALTASTATA_ACCOUNTS/$HPCS_ACCOUNT/$HPCS_PROPERTIES_FILE && echo '  Done'"

# Use same VERSION as Jupyter (version.sh)
source "$REPO_ROOT/version.sh"
echo "Building image on server (tag: $VERSION)..."
ssh $SSH_OPTS "$SSH_HOST" "cd $REMOTE_DIR && source version.sh && docker build -f openshift/rag/Dockerfile.open_llm_s390x -t altastata/rag-open-llm-s390x:latest -t altastata/rag-open-llm-s390x:\$VERSION ."
